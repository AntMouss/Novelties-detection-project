...
class WindowClassifierModel:
    ...
    def __init__(self, train_similarity_samples: List[List], nb_historic: int = 100, classification_centiles: List = None):
        """
        ...
        @param train_similarity_samples: similarity from the calculator similarity module in percent.
        """
        if classification_centiles is None:
            classification_centiles = np.array([0.02, 0.1, 0.3, 0.6, 0.9, 0.98])
        self.nb_historic = nb_historic
        self.samples_historic = train_similarity_samples[:self.nb_historic]
    ...
    def update(self , similarity_sample):

        self.samples_historic.pop()
        self.samples_historic.appendleft(similarity_sample)
        self.mean = np.mean(self.samples_historic)
        self.std = np.std(self.samples_historic)

    def predict(self , similarity_sample):

        for group , centile_value in zip(self.groups , self.centiles_values):
            if similarity_sample <= centile_value:
                return group
        return np.nan
